...
export const getAuthStatus = (apiKeys: Array<string>): RequestHandler => async (req, res) => {
  const { id } = req.query;
  const activeRequest = getActiveAuthRequest(id as string);
...
export const createAuthRequest = (authUrl: string, loginUrl: string): RequestHandler => async (req, res) => {
